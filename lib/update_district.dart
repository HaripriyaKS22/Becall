import 'dart:convert';
import 'package:becall2/update_question.dart';
...
class UpdateDistrict extends StatefulWidget {
  var id;
  UpdateDistrict({super.key, required this.id});
...
class _UpdateDistrictState extends State<UpdateDistrict> {
  ...
  Future<void> getdistrictid() async {
    try {
      ...
      setState(() {
  _selectedState = data['state'].toString(); // ✅ use state ID, not name
  questionController.text = data['name'] ?? '';
});
