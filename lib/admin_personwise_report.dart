...
class Admin_PersonwiseReport extends StatefulWidget {
  final dynamic id;

  const Admin_PersonwiseReport({super.key, required this.id});

  @override
  State<Admin_PersonwiseReport> createState() => _Admin_PersonwiseReportState();
}

class _Admin_PersonwiseReportState extends State<Admin_PersonwiseReport> {
  ...
  if (duration.contains("sec")) {
    ...
    return secMatch != null ? int.parse(secMatch.group(1)!) : 0;
  }
  ...
  if (duration.contains("min")) {
    ...
    int secs = secMatch != null ? int.parse(secMatch.group(1)!) : 0;
    return mins * 60 + secs;
  }

  // Case 3: only number → treat as seconds (ex: "660")
  if (RegExp(r'^\d+$').hasMatch(duration)) {
    return int.parse(duration);
  }

  // Case 4: decimal mm.ss (like "2.20")
 ...
 Future<void> getDateWise(DateTime from, DateTime to) async {
  ...
  try {
    ...
    if (res.statusCode == 200) {
      ...
      // Create normalized comparison range
      final start = DateTime(from.year, from.month, from.day, 0, 0, 0);
      final end   = DateTime(to.year, to.month, to.day, 23, 59, 59);

      List<Map<String, dynamic>> filtered = data
          .where((call) {
            if (call['call_datetime'] == null ||
                call['call_datetime'].toString().isEmpty) {
              return false;
            }

            final callDate =
                DateTime.tryParse(call['call_datetime'].toString());
            if (callDate == null) return false;

            // return TRUE only if call datetime is inside range
            return callDate.isAtSameMomentAs(start) ||
                callDate.isAtSameMomentAs(end) ||
                (callDate.isAfter(start) && callDate.isBefore(end));
          })
          .map((call) {
            ...
            return {
              'name': createdBy,
              'status': call['status']?.toString().replaceAll('"', '') ?? '',
      ...
      setState(() {
        ...
        print("Filtered Calls Count: ${filtered.length}");
        isLoading = false;
      });
    } else {
  ...
  for (var call in data) {
    if (call['time'] != null && call['time'].toString().isNotEmpty) {
      try {
        ...
        if (hour >= 9 && hour < 10)
          hourly['9-10 AM'] = hourly['9-10 AM']! + duration;
        else if (hour >= 10 && hour < 11)
          hourly['10-11 AM'] = hourly['10-11 AM']! + duration;
        else if (hour >= 11 && hour < 12)
          hourly['11-12 PM'] = hourly['11-12 PM']! + duration;
        else if (hour >= 12 && hour < 13)
          hourly['12-1 PM'] = hourly['12-1 PM']! + duration;
        else if (hour >= 13 && hour < 14)
          hourly['1-2 PM'] = hourly['1-2 PM']! + duration;
        else if (hour >= 14 && hour < 15)
          hourly['2-3 PM'] = hourly['2-3 PM']! + duration;
        else if (hour >= 15 && hour < 16)
          hourly['3-4 PM'] = hourly['3-4 PM']! + duration;
        else if (hour >= 16 && hour < 17)
          hourly['4-5 PM'] = hourly['4-5 PM']! + duration;
        else if (hour >= 17 && hour < 18)
          hourly['5-6 PM'] = hourly['5-6 PM']! + duration;
        else if (hour >= 18 && hour < 19)
          hourly['6-7 PM'] = hourly['6-7 PM']! + duration;
        else if (hour >= 19 && hour < 20)
          hourly['7-8 PM'] = hourly['7-8 PM']! + duration;
